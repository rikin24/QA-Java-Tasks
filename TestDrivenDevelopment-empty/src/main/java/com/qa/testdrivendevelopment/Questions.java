...
public class Questions {
	...
	public String multiChar(String input) {
		...
		for (int i=0; i<input.length(); i++) {
			...
			String index2 = Character.toString(index);
			String multiIndex = index2 + index2 + index2;
			output = output + multiIndex;
		}
	...
	public String sandwichFilling(String sandwich) {
		...
		String filling = StringUtils.substringBetween(sandwich, "bread", "bread");
		return filling;
	}
	...
	public String nMid(String input, int n) {
		...
		int diff = (n/2);
		int diffPlus = mid + diff;
		int diffMinus = mid - diff;
		StringUtils.remove(input, input.charAt(mid)); 
		for (int i=mid; i<=diffPlus; i++) {
			StringUtils.remove(input, input.charAt(i)); 
		}
		for (int i=mid; i>=diffMinus; i--) {
			StringUtils.remove(input, input.charAt(i)); 
		}
    	return input;
	}
